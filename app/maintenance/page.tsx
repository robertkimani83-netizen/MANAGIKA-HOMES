...
export default function MaintenancePage() {
  ...
  async function loadTenants(id: string) {
    const { data, error } = await supabase
      .from("tenants")
      .select(
        "id, full_name, unit_id, units!inner(unit_number, properties!inner(property_name, landlord_id))"
      )
      .eq("status", "active")
      .eq("units.properties.landlord_id", id)
  ...
  async function loadRequests(id: string) {
    ...
    const { data, error } = await supabase
      ...
      .select(
        "id, category, title, description, urgency, status, technician_name, repair_cost, created_at, tenant_id, unit_id, tenants(full_name), units!inner(unit_number, properties!inner(property_name, landlord_id))"
      )
      .eq("units.properties.landlord_id", id)
  ...
  async function addRequest() {
    ...
    if (!tenant.unit_id) {
      alert("This tenant has no unit assigned.");
      return;
    }
  ...
  async function deleteRequest(id: string) {
    ...
    const confirmed = window.confirm(
      "Are you sure you want to remove this maintenance request?"
    );
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-gray-500 mt-1">
              Track repair requests and keep your properties in good condition.
            </p>
          </div>
          ...
          <div className="mb-8 rounded-xl border bg-white p-6 shadow-sm">
            ...
            {tenants.length === 0 ? (
              <p className="text-gray-500">
                Add a tenant with a unit assigned first.
              </p>
            ) : (
