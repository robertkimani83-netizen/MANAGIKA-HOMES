...
<a href="/landlord/dashboard" className="rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-700 hover:bg-gray-50">Dashboard</a>
  ...
  <section className="mx-auto max-w-7xl px-6 py-8">
    <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      ...
      <button onClick={() => setShowForm(true)} className="rounded-lg bg-black px-5 py-3 font-medium text-white hover:bg-gray-800">+ Add Property</button>
    </div>
      ...
      <div className="mb-8 rounded-xl border bg-white p-6 shadow-sm">
        ...
        <div className="grid gap-5 md:grid-cols-2">
          <div>
            ...
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Sunrise Apartments" className="w-full rounded-lg border border-gray-300 px-4 py-3 outline-none focus:border-black" />
          </div>
          <div>
            <label className="mb-2 block text-sm font-medium text-gray-700">Location</label>
            <input type="text" value={location} onChange={(e) => setLocation(e.target.value)} placeholder="e.g. Nairobi" className="w-full rounded-lg border border-gray-300 px-4 py-3 outline-none focus:border-black" />
          </div>
        </div>
        <div className="mt-6 flex gap-3">
          <button onClick={addProperty} className="rounded-lg bg-black px-5 py-3 font-medium text-white hover:bg-gray-800">Save Property</button>
    ...
    {loading ? (
      <div className="rounded-xl border bg-white p-10 text-center shadow-sm text-gray-500">Loading properties...</div>
    ) : properties.length > 0 ? (
      <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
        {properties.map((property) => (
          <div key={property.id} className="rounded-xl border bg-white p-6 shadow-sm">
            <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-gray-100 text-2xl">🏢</div>
      ...
      <div className="rounded-xl border bg-white p-10 text-center shadow-sm">
        ...
        <p className="mx-auto mt-2 max-w-md text-gray-500">Add your first property to start managing buildings, apartments, units and tenants.</p>
        <button onClick={() => setShowForm(true)} className="mt-6 rounded-lg bg-black px-5 py-3 font-medium text-white hover:bg-gray-800">+ Add Your First Property</button>
      </div>
    )}
  </section>
