...
export default function TenantsPage() {
  ...
  // =========================================================
  // LOAD VACANT UNITS BELONGING TO THIS LANDLORD
  // =========================================================
  ...
  async function loadVacantUnits(id: string) {
    const { data, error } = await supabase
      .from("units")
      .select(
        "id, unit_number, base_rent, status, property_id, properties!inner(property_name, landlord_id)"
      )
      .eq("status", "vacant")
    ...
    if (error) {
      console.error("Vacant units error:", error);
      setVacantUnits([]);
      return;
    }
  ...
  // =========================================================
  // LOAD ONLY THIS LANDLORD'S TENANTS
  // =========================================================
  ...
  async function loadTenants(id: string) {
    ...
    const { data, error } = await supabase
      .from("tenants")
      .select(
        "id, full_name, phone_number, email, status, unit_id, units!inner(unit_number, base_rent, property_id, properties!inner(property_name, landlord_id))"
      )
      .eq("landlord_id", id)
  ...
  async function addTenant() {
    ...
    // Make sure the selected unit actually belongs to this landlord.
    if (unitId) {
      const selectedUnit = vacantUnits.find(
        (unit) => unit.id === unitId
      );
    ...
    // Mark the assigned unit occupied.
    if (unitId) {
      ...
      if (unitError) {
        console.error("Unit status error:", unitError);
      }
    }
  ...
  async function deleteTenant(tenant: Tenant) {
    ...
    // Extra protection: only delete the tenant if it belongs
    // to the currently logged-in landlord.
    ...
    // Make the tenant's unit vacant again.
    if (tenant.unit_id) {
      const { error: unitError } = await supabase
        .from("units")
      ...
      <section className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          ...
          <button
            ...
            className="px-5 py-3 rounded-lg bg-black text-white font-medium"
          >
            + Add Tenant
          </button>
              ...
              <div>
                ...
                {vacantUnits.length === 0 && (
                  <p className="mt-2 text-sm text-gray-500">
                    No vacant units available. Add units
